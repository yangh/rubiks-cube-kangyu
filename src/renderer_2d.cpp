...
void Renderer2D::draw(ImDrawList* drawList, ImVec2 offset, float scale,
    ...
    drawFace(drawList, cube.getUp(),    ImVec2(offset.x, offset.y - spacing),
            stickerSize, gap, colors);
    drawFace(drawList, cube.getLeft(),  ImVec2(offset.x - spacing, offset.y),
            stickerSize, gap, colors);
    drawFace(drawList, cube.getFront(), ImVec2(offset.x, offset.y),
            stickerSize, gap, colors);
    drawFace(drawList, cube.getRight(), ImVec2(offset.x + spacing, offset.y),
            stickerSize, gap, colors);
    drawFace(drawList, cube.getBack(),  ImVec2(offset.x + spacing * 2, offset.y),
            stickerSize, gap, colors);
    drawFace(drawList, cube.getDown(),  ImVec2(offset.x, offset.y + spacing),
            stickerSize, gap, colors);
}
